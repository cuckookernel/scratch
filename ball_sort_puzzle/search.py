...
# ______________________________________________________________________________path_costpath_cost
        ...
        if problem.goal_test( node.state ):
            ...
            if display:
                print( len( explored ), "paths have been expanded and", len( frontier ),
                       f"paths remain in the frontier - ({tm1 -tm0:.3f} seconds)" )
        ...
        if self.problem.goal_test( s1 ):
            ...
            if self.s is not None:
                ...
                self.H[self.s] = min( self.LRTA_cost( self.s, b, self.problem.output( self.s, b ),
                                                      self.H ) for b in
                                      self.problem.actions( self.s ) )

            # an action b in problem.actions(s1) that minimizes costs
            self.a = min( self.problem.actions( s1 ),
                          key=lambda b: self.LRTA_cost( s1, b, self.problem.output( s1, b ),
                                                        self.H ) )
...
def genetic_algorithm( population, fitness_fn, gene_pool=[0, 1], f_thres=None, ngen=1000,
                       pmut=0.1 ):
    """[Figure 4.8]"""
    for i in range( ngen ):
